...
while ($row = mysqli_fetch_array($rsd)) {
	...
	while ($x < $teamSize) {
	    ...
	    <div class='callout moroom'>
	    	<a onClick='getStats(event);showPlayerStats()'id=".$playerId[$x]." href='#statsModal'><span id=".$playerId[$x]." class='badge playerIcons'>
	    		<i id=".$playerId[$x]." class='fi-power'></i></span></a>
	    	<a onClick='getInfoForm(event);showPlayerInfo()'id=".$playerId[$x]." href='#infoFormModal'><span id=".$playerId[$x]." class='warning badge playerIcons'>
	    		<i id=".$playerId[$x]." class='fi-power'></i></span></a>
   ...
   <?php include 'connection/database.php';
    ...
    <p onclick="hidePlayerStats()" title="Close" class="alert close">X</p>
	...
	<div id="playerInfo" class="">
	 <div>
    <a href="#" title="" onclick="hidePlayerInfo()" class="close">X</a>
	  <?php
	  include 'connection/database.php';
	 global $db;
	 $sql = "SELECT * FROM teams WHERE teamName = '$_SESSION[teamName]'";
	 ...
	 Player Id:<input type="text" id='playerId'>
	  ...
	  <div class="column small-6">
	  	<a href="#" onclick="hidePlayerInfo();" title="" class="button alert">Cancle</a>
	    <a href="#"><input class='button' onclick='updateRoster();hidePlayerInfo()'name='addplayer'id='addPlayerBtn' value='Update Player'></a>
	  </form>
	 </div>
	</div>